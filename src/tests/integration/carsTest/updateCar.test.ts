...
chai.use(chaiHttp);
const { expect } = chai;
const request = (id = carMock.validCarWithId._id) => chai
  .request(app).put(`/cars/${id}`).send()

describe("Endpoint /cars/:id do tipo put", () => {
  beforeEach(() => {
    sinon.restore()
  })

  it('É possível atualizar 1 carro pelo id com sucesso', async () => {
    const deleteStub = sinon.stub(Model, 'findByIdAndUpdate').resolves(carMock.validCarWithId)
